...
input_file <- file.path(meth_dir, "adultfemale_05_final_coverage.txt")
sample_name <- "adultfemale_05"

df <- read_delim(input_file, "\t", escape_double = FALSE, col_names = TRUE, trim_ws = TRUE)

annotation <- read_table("all_features.tsv")
annotation <- annotation[, c("chr", "start", "end", "type", "feature")]
...
output <- output[, -c(1,2)]
check <- summaryBy(total_coverage + count_c ~ chr + feature + type + start + end, data=output, FUN=sum)
