...
# Standardize feature labels
all <- all %>%
  mutate(feature = ifelse(feature == "promoter_2000bp", "promoter", feature))
...
new_levels <- c("promoter", "exon", "intron", "intergenic", "TE")
...
### ---- Save the final plot ---- ###
plot_file <- file.path(dir, "WML_with_feature_counts.pdf")
